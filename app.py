...
def get_uniqueid():
    ...
    try:
        ...
        logging.exception("Greška pri dohvaćanju UNIQUEID")
...
def get_idblag_for_date(date_str):
    ...
    try:
        ...
        logging.exception(f"Greška pri dohvaćanju IDBLAG za datum={date_str}")
        return None
...
def get_blagajna_stanje(id_blag):
    ...
    try:
        ...
        logging.exception(f"Greška pri dohvaćanju BLAGAJNA_STANJE za IDBLAG={id_blag}")
        return []
...
def get_kupovni_tecaj(date_str, valuta):
    ...
    try:
        ...
        logging.exception(
            f"Greška pri dohvaćanju KUPOVNI_TECAJ za datum={date_str}, valuta={valuta}"
        )
        return 1.0
...
def get_all_kupovni_tecajevi_for_date(date_str):
    ...
    try:
        ...
        logging.exception(f"Greška pri dohvaćanju svih tečajeva za datum={date_str}")
        return {}
...
def get_transactions_for_idblag(id_blag):
    ...
    try:
            ...
            transactions.append(
                {
                    ...
                    "prodaoime": row[9],
                    "prodaodok": row[10],
                    "akronim_transakcije": row[11],
                    "provbanke": row[12] if row[12] else 0,
                    "korisnik_ime": row[13],
                }
            )
        ...
        logging.exception(f"Greška pri dohvaćanju transakcija za IDBLAG={id_blag}")
        return []
    ...
    try:
        ...
        while current_date <= end_date:
            ...
            for stanje in stanje_rows:
                ...
                ET.SubElement(valto_tetel, "valto_valuta").text = (
                    str(valuta) if valuta else ""
                )
                ET.SubElement(valto_tetel, "valto_nyito").text = f"{float(iznos):.2f}"
                ET.SubElement(valto_tetel, "valto_nyito_km").text = (
                    f"{valto_nyito_km:.2f}"
                )
                ET.SubElement(valto_tetel, "valto_exc_percent").text = (
                    f"{100 - float(prov_za_banku):.2f}"
                )
                ET.SubElement(valto_tetel, "valto_bank_percent").text = (
                    f"{float(prov_za_banku):.2f}"
                )
            ...
            for t in transactions:
                ...
                # ═══════════════════════════════════════════════════════════
                # NOVI KOD: Provjera AKRONIM_TRANSAKCIJE za STORNO
                # ═══════════════════════════════════════════════════════════
                akronim = t.get("akronim_transakcije", "")
                if akronim and str(akronim).upper().startswith("STORNO"):
                    ET.SubElement(kozonseges_tetel, "storno").text = "STORNO"
...
title_label = tk.Label(
    root, text="Export Blagajničkih Transakcija", font=("Arial", 16, "bold")
)
